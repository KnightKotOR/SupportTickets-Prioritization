    ...
    def __call__(self, trial):
        ...
        if clf_name == "XGBClassifier":
            ...
            clf_obj = XGBClassifier(
                max_depth=max_depth, n_estimators=500, learning_rate=lr, reg_alpha=reg_alpha,
                reg_lambda=reg_lambda, subsample=subsample, colsample_bytree=colsample,
                min_child_weight=min_child_weight, objective="multi:softprob", gamma=gamma,
    ...
    def fit(self, x, y, x_val, y_val, n_trials=100, n_startup_trials=20):
        ...
        for model in self.models_list:
            ...
            study = optuna.create_study(
                study_name=f"optimization_{model}",
                direction="maximize",
                sampler=sampler
            )
